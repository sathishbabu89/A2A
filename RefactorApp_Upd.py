...
class UI:
    ...
    @staticmethod
    def show_generation_buttons() -> Tuple[bool, bool]:
        """Display documentation options and return button states"""
        col1, col2 = st.columns(2)
        with col1:
            basic = st.button("📝 Generate Basic Documentation")
        with col2:
            detailed = st.button("🔍 Generate Detailed Documentation")
    ...
    def show_progress(current: int, total: int):
        ...
        st.caption(f"Processing file {current} of {total}")
    
    @staticmethod
    def show_results():
        """Display generated documentation"""
        if not st.session_state.documentation:
            st.info("Upload code and generate documentation to view results")
            return
        
        st.subheader("Documentation Results")
...
def main():
    ...
    basic_btn, detailed_btn = UI.show_generation_buttons()
    
    if uploaded_file and (basic_btn or detailed_btn) and api_key:
